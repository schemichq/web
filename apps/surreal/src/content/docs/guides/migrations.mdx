...
Seeds live in `database/seed/` — TypeScript files that each export a default `(db) => Promise<…>` taking the connected client (annotate it with `Surreal`). `schemic seed` runs `index.ts` (or every seed in filename order if there is none); `schemic seed <name>` runs one; `--all` runs them all. A leading `NN-` prefix orders a seed but is not part of its name (`01-users.ts` is the seed `users`). Raw SurrealQL files load into a seed via an import attribute — `import surql from "./schema.surql" with { type: "text" }` (typed by the scaffolded `seeds.d.ts`):
