...
There is no `push` (apply-without-migration) or `pull` (introspect-into-code) on the Postgres driver yet, and `diff` is **structural only** — it has no `--live`, `--ts`, or `--watch` mode. The path through migrations is the supported way to change the database. See the [CLI reference](/docs/reference/cli) for exactly which commands ship.
</Callout>
...
The diff today is **structural**: it detects table create/drop, `ADD COLUMN` / `DROP COLUMN`, column type changes (best-effort cast), nullability changes, and index and foreign-key add/drop. Plain and unique btree indexes and foreign keys round-trip cleanly, so adding or dropping one produces a real, accurate migration.

Expression-level clauses — `DEFAULT`, `CHECK`, `GENERATED`, and `COMMENT` — are **emitted faithfully but excluded from the diff**, because Postgres rewrites those expressions on read and an exact comparison would produce phantom migrations. The practical consequence: changing *only* a default or a check does not generate a migration. Set those when you first author a column, or recreate the column when you need to change one. See [from schema to DDL](/docs/concepts/schema-to-ddl) for the full list.
