...
The scaffolded config imports `@schemic/core/config`, and pnpm's strict `node_modules` doesn't expose that transitively — add it directly: `pnpm add @schemic/core`. Bun, npm, and yarn hoist it, so they don't need it.
</Callout>
...
<Terminal command="npx schemic init" output={`  + schemic.config.ts
  ...
  + database/seed/seeds.d.ts
  + .env.example
  + database/migrations/meta/_snapshot.json

✓ Initialized. Edit database/schema, then run \`schemic gen\`.`} />

`init` writes a `schemic.config.ts` (where your database connection lives), a sample `user` schema, a `database/seed/` folder, and an empty migration snapshot. It never overwrites files that already exist.
