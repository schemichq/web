...
In the Postgres driver, codecs are **per-field**: you encode and decode one column at a time through `table.fields.<column>`. There is no whole-row `Table.encode` / `Table.decode` helper yet — that is a known gap, tracked for a future release.
...
const db = new PGlite("file:./.pgdata"); // or "" for in-memory
```
...
Build the query with the `pgSql` tagged template — interpolated values become bind params automatically, and `identifier(...)` safely quotes table and column names. Then decode each column through its field:
...
const accounts = rows.map((r) => ({
  id: r.id,
  email: account.fields.email.decode(r.email),
  balance: account.fields.balance.decode(r.balance), // "1299" -> Money
}));
