...
On the Postgres driver, `diff` compares structure — tables, columns, types, nullability, indexes, and foreign keys — always against the live database (it introspects it). The `--live`, `--ts`, and `--watch` flags are no-ops here. Changes to `DEFAULT`, `CHECK`, `GENERATED`, or `COMMENT` are excluded from migration generation by design, though the `diff` preview may still *display* one — see [the migration model](/docs/concepts/migrations-model).
</Callout>
...
<Terminal command="npx schemic gen add_bio" output={`1 change — 1 Field.
...
✓ 20260626114301_add_bio.sql  (+1 up / 1 down)`} />
...
<Terminal command="npx schemic migrate" output={`  ↑ 20260626114301_add_bio
...
<Terminal command="npx schemic status" output={`  ✓ applied  20260601000000_initial
  · pending  20260626114301_add_bio
