...
`create schemic` is interactive: it asks for a project directory and a database driver, installs everything, and scaffolds your project. Run it in an empty directory for a new project, or inside an existing one — it merges the `@schemic` dependencies and `db:gen` / `db:migrate` scripts into your `package.json` without touching the rest of your setup. For the SurrealDB driver it scaffolds:
...
`bunx schemic init` (or `npx schemic init` / `pnpm dlx schemic init`) runs the same setup in place — installing and scaffolding into your current directory. After it runs you have local `schemic` and `sc` bins, plus `db:gen` / `db:migrate` scripts.
</Callout>
